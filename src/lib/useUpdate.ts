...
import type { UpdatePolicy } from "./api";

/**
 * Where the update conversation has got to.
 *
 * `quiet` covers every uninteresting case at once — no newer version, checking
 * turned off, the machine offline — because they all mean the same thing to the
 * person looking at the window: nothing to do.
 */
...
export function useUpdate() {
  ...
  const readPolicy = useCallback(async () => {
    const next = await api.updatePolicy();
    setPolicy(next);
    return next;
  }, []);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // Without the policy we do not know whether we are allowed to look, and
        // the safe reading of that is: do not.
        return;
      }
      ...
      try {
        ...
        // Offline, GitHub having a bad day, a manifest that has not been
        // published yet. None of it is the user's problem and none of it should
        // interrupt someone who opened Tessera to copy a code.
      }
    })();
  ...
  const install = useCallback(async () => {
    ...
    try {
      ...
      // A cancelled administrator prompt lands here too, which is why the
      // failure always offers the download page rather than only an apology.
  ...
  return { policy, phase, install, dismiss, setEnabled };
}
