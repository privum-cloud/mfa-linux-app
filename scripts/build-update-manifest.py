...
The plugin looks for `{os}-{arch}-{installer}` before falling back to
`{os}-{arch}`, so one manifest can carry a different artefact for each package
format. That is what lets someone who installed the .deb be handed a .deb
rather than an AppImage they cannot install.

Signatures are read from the .sig files the bundler produced, and the download
URL is derived from the asset each one signs. Nothing here invents a filename:
if a package was not built and signed, it simply does not appear, and the
updater tells that platform there is nothing for it rather than handing it the
wrong file.
"""
